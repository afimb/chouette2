class InitReferentialLastUpdate < ActiveRecord::Migration[4.2]
...
BEGIN
  ...
  -- init last update timestamp to current time
  execute 'insert into ' || dest_schema  || '.referential_last_update(last_update_timestamp) values(current_timestamp)';
